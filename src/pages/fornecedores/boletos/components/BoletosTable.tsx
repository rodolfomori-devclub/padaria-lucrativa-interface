  ...
  if (boletos.length === 0) {
    return (
      <TableEmptyState
        ...
        description="Comece cadastrando o primeiro boleto clicando no botão 'Novo Boleto'."
      />
    );
  }
            ...
            <TableRow key={boleto.id}>
              <TableCell className="font-medium">
                <div className="flex items-center gap-2">
                  {boleto.recurringTemplateId && (
                    <Repeat
                      className="h-4 w-4 text-blue-500"
                      aria-label="Boleto recorrente"
                    />
                  )}
                  {boleto.supplier.name}
                </div>
              </TableCell>
              <TableCell>{formatCurrency(boleto.value)}</TableCell>
              <TableCell>{formatDateDDMMYYYY(boleto.dueDate)}</TableCell>
              <TableCell>
                <span
              ...
              <TableCell>{formatDateDDMMYYYY(boleto.paymentDate)}</TableCell>
              <TableCell
                className="max-w-52 truncate"
                title={boleto.observations || "-"}
              >
                {boleto.observations || "-"}
              </TableCell>
